      ...
      {isCreateDeckModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl border-2 border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-5">
             <div className="flex items-center justify-between">
                <h3 className="font-black text-lg text-slate-900 dark:text-white">Tạo Bộ Thẻ Mới</h3>
                <button onClick={() => setIsCreateDeckModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X /></button>
             </div>
             <form onSubmit={handleCreateDeckSubmit} className="space-y-4">
                <input
                  type="text"
                  placeholder="Tên bộ thẻ (ví dụ: Từ vựng IELTS)"
                  className="w-full px-4 py-3 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-sm font-bold"
                  value={newDeckTitle}
                  onChange={(e) => setNewDeckTitle(e.target.value)}
                />
                <textarea
                  placeholder="Mô tả ngắn gọn..."
                  className="w-full px-4 py-3 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 text-sm font-bold min-h-[100px]"
                  value={newDeckDesc}
                  onChange={(e) => setNewDeckDesc(e.target.value)}
                />
                <button type="submit" className="w-full py-3.5 rounded-2xl bg-[#1E2540] text-white font-black text-xs uppercase tracking-wider shadow-md">
                  TẠO NGAY
                </button>
             </form>
          </div>
